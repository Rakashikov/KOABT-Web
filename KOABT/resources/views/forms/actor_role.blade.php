<form action="{{isset($route)?$route:route('actor_roles.store')}}" method="POST" >
    {{csrf_field()}}
...
<div class="form-group">
    ...
    <select class="form-control {{ $errors->has('actor_id') ? ' is-invalid' : '' }}" name="actor_id" id="actor_id">
        @if(isset($actors))
@foreach ($actors as $data)
<option value="{{$data->id}}" {{$data->id==$model->actor_id?'selected':''}}>{{$data->id}}</option>
@endforeach
@endif
